...
import { Lightning, Info, Wallet, Gear, CaretDown, CaretUp, HandPalm, ChartLine, Bell, SignIn, SignOut } from "@phosphor-icons/react";
...
import IntradaySignalChart from "./IntradaySignalChart.jsx";
...
const BREAKOUT_SETUP_TYPES = new Set(["BREAKOUT", "BREAKOUT_RETEST"]);

/**
 * Five "best of" cards from the candidates that got real setup detection
 * (signal != null — only the top-ranked shortlist, not the full ranking
 * table). Each returns the winning candidate or null if none qualify.
 */
function computeTopCards(candidates) {
  const withSignal = candidates.filter((c) => c.signal);
  const best = (list, keyFn) => list.reduce((acc, c) => (keyFn(c) != null && (acc == null || keyFn(c) > keyFn(acc)) ? c : acc), null);

  return {
    bestLong: best(withSignal.filter((c) => c.direction === "LONG"), (c) => c.signal.score),
    bestShort: best(withSignal.filter((c) => c.direction === "SHORT"), (c) => c.signal.score),
    fastestMomentum: best(withSignal, (c) => c.signal.momentumScore),
    bestRR: best(withSignal.filter((c) => c.signal.riskReward != null), (c) => c.signal.riskReward),
    earlyBreakout: best(withSignal.filter((c) => BREAKOUT_SETUP_TYPES.has(c.signal.setupType)), (c) => c.signal.score),
  };
}
...
export default function IntradayTrader() {
  ...
  const load = useCallback(() => {
    Promise.all([
      ...
      fetch("/api/intraday?resource=signals").then((r) => r.json()),
    ])
      .then(([scanBody, posBody, sigBody]) => {
    ...
    <div className="p-4 max-w-[1400px] mx-auto">
        ...
        {settingsOpen && (
          <div className="px-3 pb-3 pt-1" style={{ borderTop: "1px solid var(--c-line)" }}>
            <div className="flex items-center gap-4 flex-wrap py-2.5">
              ...
              <label className="flex items-center gap-1.5 text-[11.5px]">
                <input type="checkbox" checked={!!settingsDraft.liquidity_grab_enabled} onChange={(e) => setField("liquidity_grab_enabled", e.target.checked)} />
                Liquidity Grab
              </label>
              <label className="flex items-center gap-1.5 text-[11.5px]">
                Timeframe
                <select
                  value={settingsDraft.liquidity_grab_interval ?? "3minute"}
                  onChange={(e) => setField("liquidity_grab_interval", e.target.value)}
      ...
      {loading ? (
        ...
        <>
          ...
          {(topCards.bestLong || topCards.bestShort || topCards.fastestMomentum || topCards.bestRR || topCards.earlyBreakout) && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5 mb-4">
              {[
                { label: "Best Long", cand: topCards.bestLong, metric: (c) => `Score ${c.signal.score}` },
                { label: "Best Short", cand: topCards.bestShort, metric: (c) => `Score ${c.signal.score}` },
          ...
          <h2 className="text-[13px] font-bold mt-5 mb-2">
            Liquidity Grab <span className="font-normal text-muted">(stop-run reversal, 3-min · min 1:2 R:R)</span>
          </h2>
          {liquidityGrabHits.length === 0 ? (
            <p className="text-[12.5px] text-muted py-6 text-center">No liquidity grab set-ups on the current shortlist right now.</p>
          ) : (
              ...
              <table className="w-full text-[12px]">
                <thead>
                  <tr className="text-muted text-left" style={{ background: "var(--c-surface-2)" }}>
                    ...
                    <th className="font-medium py-2 pr-2">Score</th>
                    <th className="font-medium py-2 pr-2">Direction</th>
                    <th className="font-medium py-2 pr-2">Swept Level</th>
                ...
                <tbody>
                  {liquidityGrabHits.map((c) => {
                    const lg = c.liquidityGrab;
                    return (
                      <tr key={c.symbol} style={{ borderTop: "1px solid var(--c-line)", background: lg.status === "SIGNAL_CONFIRMED" ? "var(--c-gain-soft, transparent)" : undefined }}>
                        <td className="py-2 pl-3 pr-2">
                          <IntradaySignalChart symbol={c.symbol} candidate={{ signal: lg }}>
                            <div className="font-semibold">{c.symbol}</div>
                          </IntradaySignalChart>
                          <div className="text-[10.5px] text-faint">{c.sector ?? "—"}</div>
                        </td>
                        <td className="py-2 pr-2"><ScoreBadge score={lg.score} /></td>
                        <td className={`py-2 pr-2 text-[11px] font-medium ${lg.direction === "LONG" ? "text-gain" : "text-loss"}`}>{lg.direction}</td>
                        <td className="py-2 pr-2 text-right n text-[11px]">{inr(lg.sweptLevel)}</td>
                        <td className="py-2 pr-2 text-right n">{inr(lg.entry)}</td>
                        <td className="py-2 pr-2 text-right n">{inr(lg.stop)}</td>
                        <td className="py-2 pr-2 text-right n">{lg.riskReward != null ? lg.riskReward.toFixed(1) : "—"}</td>
                        <td className="py-2 pr-3 text-[11px]">
                          <span className={`font-semibold ${toneClass(SIGNAL_TONE[lg.status])}`}>{SIGNAL_LABEL[lg.status] ?? lg.status}</span>
                          {lg.confidence && <span className="text-faint"> · {lg.confidence.replace("_", "+")}</span>}
          ...
          <h2 className="text-[13px] font-bold mt-5 mb-2">Alert Timeline</h2>
          {timeline.length === 0 ? (
            <p className="text-[12.5px] text-muted py-6 text-center">No confirmed signals or position events yet.</p>
          ) : (
