...
test('scores a well-formed candidate with every component present and a sensible weighted score', () => {
  ...
  const c = condor(s);
  const independentEv = {
    expectedValue: 500, evPerUnitRisk: 0.08,
    pop: { probability: 0.78, method: 'empirical' as const, sampleCount: 120, comparison: null, disagreementPct: null },
  };
  const { score, components, missingComponents } = scoreTradeQuality(c, s, { ivRank: 72, marginRequired: c.maxProfit / 0.1, premiumEdgePct: 12, independentEv });
  ...
  assert.ok(components.independentEv !== null && components.independentEv >= 0 && components.independentEv <= 100);
...
test('a missing IV rank and no margin figure are excluded, not treated as zero — the remaining weights renormalize', () => {
  ...
  assert.deepEqual(missingComponents.sort(), ['ivRank', 'independentEv', 'marginEfficiency', 'premiumEdge'].sort());
  assert.equal(components.ivRank, null);
  assert.equal(components.premiumEdge, null);
  assert.equal(components.independentEv, null);
